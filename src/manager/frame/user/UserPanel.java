    ...
    public UserPanel() {
        ...
        parentMenu.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                setContent(new manager.frame.admin.DataTablePanel());//切换到检查项管理面板
            }
        });
        ...
        JMenu orderMenu = new JMenu("体检预约");
        orderMenu.addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                // 这里假设有全局当前用户对象 SystemConstants.currentUser
                setContent(new manager.frame.user.OrderPanel(SystemConstants.currentUser));
            }
        });
